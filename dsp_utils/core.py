...
def get_fft_size(frame_size: int, ir_size: int, power_of_2: bool = True) -> int:
  ...
  if power_of_2:
    # Next power of 2.
    fft_size = int(2**tf.math.ceil(tf.math.log(tf_float32(convolved_frame_size)) / tf.math.log(2.0)))
  else:
...
def crop_and_compensate_delay(audio: tf.Tensor, audio_size: int, ir_size: int,
  ...
  total_size = int(tf.shape(audio)[-1])
...
def fft_convolve(audio: tf.Tensor,
  ...
  audio_size = int(tf.shape(audio)[1])

  # Add a frame dimension to impulse response if it doesn't have one.
  ir_shape = tf.shape(impulse_response)
  if len(ir_shape) == 2:
  ...
  ir_shape = tf.shape(impulse_response)
  batch_size_ir, n_ir_frames, ir_size = int(ir_shape[0]),int(ir_shape[1]),int(ir_shape[2])
  """
  ...
  frame_size = int(tf.math.ceil(audio_size / n_ir_frames))
  hop_size = frame_size
  audio_frames = tf.signal.frame(audio, frame_size, hop_size, pad_end=True)

  # Check that number of frames match.
  n_audio_frames = int(tf.shape(audio_frames)[1])
  """
...
def apply_window_to_impulse_response(impulse_response: tf.Tensor,
  ...
  window = tf.broadcast_to(window, tf.shape(impulse_response))
